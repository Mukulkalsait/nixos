...
// Your Kitty settings as constants
const float DECAY_X = 0.25;      // Matches cursor_trail_decay x
const float DECAY_Y = 0.5;       // Matches cursor_trail_decay y
const float THICKNESS_X = 1.0;   // Matches cursor_trail_thickness x
const float THICKNESS_Y = 1.0;   // Matches cursor_trail_thickness y
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // Apply decay and thickness to trail
    float trailAlpha = antialising(sdfTrail);
    trailAlpha *= exp(-lineLength * DECAY_X) * THICKNESS_X; // Apply X decay/thickness
    trailAlpha *= exp(-abs(vu.y - centerCC.y) * DECAY_Y) * THICKNESS_Y; // Apply Y decay/thickness

    vec4 newColor = vec4(fragColor);
    vec4 trail = TRAIL_COLOR;
    trail = saturate(trail, 2.5);
    trail.a *= trailAlpha * OPACITY;
